...
namespace Tinkoff_Invest.WebApi.Services;

public class UsersServiceSample
{
    private readonly UsersService.UsersServiceClient _grpcClient;

    public UsersServiceSample(InvestApiClient investApiClient)
    {
        _grpcClient = investApiClient.Users;
    }

    public async Task<string> GetUserInfoDescriptionAsync(CancellationToken cancellationToken)
    {
        var accountsResponse = await _grpcClient.GetAccountsAsync(cancellationToken);
        var infoResponse = await _grpcClient.GetInfoAsync(cancellationToken);
        var userTariffResponse = await _grpcClient.GetUserTariffAsync(cancellationToken);

        GetMarginAttributesResponse? marginAttributesResponse = null;
        try
        {
            marginAttributesResponse = await _grpcClient.GetMarginAttributesAsync(cancellationToken);
        }
        catch (Exception)
        {
            // ignored
    ...
    {
        var accountsResponse = _grpcClient.GetAccounts();
        var infoResponse = _grpcClient.GetInfo();
        var marginAttributesResponse = _grpcClient.GetMarginAttributes();
        var userTariffResponse = _grpcClient.GetUserTariff();
